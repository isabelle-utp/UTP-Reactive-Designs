...
definition mk_RD :: "'s pred \<Rightarrow> ('t::trace \<Rightarrow> 's pred) \<Rightarrow> ('t \<Rightarrow> 's hrel) \<Rightarrow> ('s, 't, 'a) rsp_hrel" where
"mk_RD P Q R = \<^bold>R\<^sub>s(\<lceil>P\<rceil>\<^sub>S\<^sub>< \<turnstile> (\<Sqinter> x. \<lceil>Q(x)\<rceil>\<^sub>S\<^sub>< \<and> ($tr\<^sup>> = $tr\<^sup>< + \<guillemotleft>x\<guillemotright>)\<^sub>e) \<diamondop> (\<Sqinter> x. \<lceil>R(x)\<rceil>\<^sub>S\<and> ($tr\<^sup>> = $tr\<^sup>< + \<guillemotleft>x\<guillemotright>)\<^sub>e))"

definition trace_pred :: "('t::trace \<Rightarrow> 's pred) \<Rightarrow> ('s, 't, '\<alpha>) rsp_hrel" where
[pred]: "trace_pred P = (\<Sqinter> x. [P x]\<^sub>S\<^sub>< \<and> (\<guillemotleft>x\<guillemotright> = $tt)\<^sub>e)"
...
lemma SRD_mk_RD [closure]: "[P \<turnstile> Q(trace) | R(trace)]\<^sub>R is SRD"
  apply (simp add: mk_RD_def)
  apply (rule closure)
    
  apply (simp_all add: unrest closure unrest_ssubst_expr usubst_eval usubst)
